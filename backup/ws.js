...
function WsConnector (id, port) {
  ...
  if (port != null) {
    const server = https.createServer({
      cert: fs.readFileSync('/home/acme/.acme/domains/anoma.ch/fullchain.pem'),
      key: fs.readFileSync('/home/acme/.acme/domains/anoma.ch/privkey.pem')
    })
    server.listen(port, '213.239.214.178')
    self._wss = new WebSocket.Server({ server })
    self._wss.on('connection', onConnection)
    self._wss.on('listening', onListen)
    if (port !== 0) self.url = 'wss://213.239.214.178:' + port
  }
  ...
  function onListen () {
    console.log("onListen", self._wss._server.address())
    if (self.destroyed) return
    self.url = 'wss://213.239.214.178:' + self._wss._server.address().port
  }
}
